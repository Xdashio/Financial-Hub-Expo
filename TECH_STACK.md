...
| Auth | **Supabase Auth** support biometric-adjacent flows (device-level biometric + session token) cheaply. |
...
| Behavioral scoring / plan assignment logic | **Rules engine in your own backend code first, not ML** | The plan-assignment logic described (income pattern × allocation style → 1 of 4 plans) is a decision tree from onboarding answers, not something that needs a trained model yet. Keep it deterministic and inspectable for MVP — this also makes the "why this plan" explanation screen trivial to generate honestly, since you can literally show which rule fired. Future versions may evolve toward ML-based pattern recognition for income detection and adaptive allocation. |
| Income detection (future) | **Webhook integration or periodic polling** | For future bank/PSP integration, income detection will need webhooks or polling to classify incoming funds (salary, business income, transfer, irregular) and trigger automatic allocation options. This is not needed for MVP manual entry but should be considered in API design. |
| Daily spending engine | **Configurable rule engine with adaptive logic** | Daily Budget plans require behavior-aware packet allocation that varies by user profile (student vs salaried vs freelancer). This needs a flexible rule engine that can handle different daily packet patterns and adaptive budgeting based on runway and income timing. |
| Financial outcomes tracking | **Event-based analytics system** | The system needs to track specific financial health outcomes (emergency resilience, goal-based savings, investment readiness, debt reduction, capital accumulation). This requires structured event logging and metrics that feed both user-facing insights and operational analytics. |
...
| Billing | **paystack Billing (usage-based) or a metering table you own + paystack for invoicing** |  supports usage-based billing well if pricing ends up being per-active-end-user or per-transaction; keep your own metering event log regardless so you're not solely dependent on a third party for the source of truth. Pricing model itself is not agreed yet — see `PRD.md` §5. |
...
- **No NoSQL as primary store**: money/pocket/reallocation data is inherently relational and needs transactional guarantees (a reallocation must debit one pocket and credit another atomically, even when simulated) — Postgres is the right default here.
