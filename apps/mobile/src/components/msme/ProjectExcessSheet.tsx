...
export function ProjectExcessSheet({ visible, onClose, projectId, projectName, prompt, onSuccess }: Props) {
 ...
 <Text style={{ ...typography.heading, color: colors.ink }}>{projectName}</Text> received <Text style={{ ...typography.heading, color: colors.gold }}>{fmt}</Text> more than its planned funding targets. Excess is never moved silently — choose where it should go.
 </Text>
 ...
 All tiers are already fully funded. Directing excess to Needs/Wants will overfund that tier; Savings moves it to your MSME Savings pocket (requires a second confirmation); Keep leaves it as project surplus until completion.
 </Text>
 </View>
 </View>
 ...
 You chose Savings. This moves <Text style={{ ...typography.heading, color: colors.plum }}>{fmt}</Text> out of the project into your MSME Savings pocket. Tap again to confirm — this requires explicit confirmation.
 </Text>
 </View>
 )}
