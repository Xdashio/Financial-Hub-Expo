...
export function ProjectCompleteSheet({ visible, onClose, projectId, project, completeInfo, onSuccess }: Props) {
 ...
 {hasRemaining
 ? `${fmt} remains across tiers — the system will not silently move it. Choose Keep or Move to Savings.`
 ...
 You chose Savings. This moves <Text style={{ ...typography.heading, color: colors.plum }}>{fmt}</Text> to your MSME Savings pocket. Tap again to confirm — explicit confirmation is required.
 </Text>
 </View>
 )}
 ...
 The system must not silently transfer or remove remaining funds — this action is always explicit.
 </Text>
 </BottomSheetModal>
 );
}
