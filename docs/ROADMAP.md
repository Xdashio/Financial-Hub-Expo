...
> - **`income_events.label` was `NOT NULL`** in the schema despite being coded as optional in `CreateIncomeDto` — crashed every manual income entry submitted without a label. Fixed: column is now nullable.
